...
export async function waitForCompletedTurn(input, { timeoutMs = 60000, pollIntervalMs = 100 } = {}) {
...
export async function capture(input, { send = true, defer = false } = {}) {
  ...
  if (!binding) {
    const first = input.turn_id ? parsed.turns.find(turn => turn.turnId === input.turn_id)
      : defer ? parsed.turns.at(-1) : parsed.turns.findLast(turn => turn.completed);
  ...
  if (defer) {
    const turn = input.turn_id ? parsed.turns.find(turn => turn.turnId === input.turn_id) : parsed.turns.at(-1);
    if (!turn) throw new Error('The hook turn is not available in the rollout yet.');
    const status = { enabled: true, phase: 'waiting-for-turn-complete', turnId: turnKey(turn), target: config.targetName, updatedAt: new Date().toISOString() };
    saveJson(join(config.data_directory, 'status.json'), status);
    // Codex persists task_complete after Stop returns. The worker must not retain the hook's pipes.
    const child = spawn(process.execPath, [fileURLToPath(import.meta.url), '--after-stop'], {
      detached: true, stdio: ['pipe', 'ignore', 'ignore'],
    });
    await new Promise((resolve, reject) => {
      child.once('error', reject); child.stdin.once('error', reject);
      child.stdin.end(JSON.stringify({ ...input, waitTurn: turnKey(turn) }), resolve);
    });
    child.unref();
    return status;
  }
...
if (isMain(import.meta.url)) {
  try {
    ...
    const result = await capture(input, { send: !process.argv.includes('--preview'),
