...
| Session | 两轮主任务对应同一个 Session；`missingSession=0` | 存在缺失或不匹配的会话字段，下一阶段前补映射 |
| Turn | 两次用户输入对应两条主任务 Trace | 标题等辅助 Trace 可能额外出现；不能机械要求全局 traces 恰好为 2 |
| 真实时间 | 3 秒工具的 span 耗时符合实际 | 工具 span 或关联未完整采集 |
| Hook | hooks 至少出现 UserPromptSubmit、PreToolUse、PostToolUse、Stop | 环境未继承、插件目录未加载、Hook 执行器未找到 Node 或事件未触发 |
