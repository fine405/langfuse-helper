...
最近一次[真实桌面验收](acceptance-2026-09-08.md)通过。持久插件安装和按 Trace ID 的跨批 Session 关联已修复；阶段 2 可以开始。

先按 README 启动 Collector，完全退出 WorkBuddy 后安装插件，再通过诊断入口启动 WorkBuddy，并在同一个新任务中完成两次请求。使用无敏感信息的测试内容。
...
`npm run status` 是聚合统计。如果要核对具体 span，可运行 `npm run preview:export` 读取 SQLite 中的关联结果，按 `langfuse.session.id` 和 `span.type` 找到测试主任务。字段转换保留了 traceId、spanId、parentSpanId 和原始纳秒时间。`.local/collector/traces.jsonl` 是关联前的元数据副本，缺少 Session 的原生子 span 在这个副本中仍保留缺失，便于核查。
...
3. Hook 有、native 无：插件加载有效，但原生 exporter 尚未到达本机，检查主任务模式、worker 和环境继承。
4. native 有、Hook 无：检查 Node 是否可执行、持久插件是否启用、是否从诊断入口启动及 Hook 支持；这两条采集路径相互独立。
5. 同时存在其他 WorkBuddy worker：先只用一个新任务验证，记录多 worker 情况，避免将辅助引擎数据当作主任务。

阶段边界：这版不是完整 Langfuse 上报器，不包含 JSONL 轨迹重建、面向 Langfuse 的发送队列、运行中卡顿判定或历史补传。阶段 1 的验证结果决定阶段 2 是否继续用原生 OTLP，或增加必要的 transcript 适配。

关联器按 traceId 保存 Session 映射，允许子 span 先到、跨批次和重启后到达。Session 冲突时整条 Trace 标记为 conflict，不猜测归属；无法关联的记录保留在 pending/unassociated 并出现在状态统计中。Collector 到本地关联器的队列已持久化，但这不等于 Langfuse 上报与远端去重已经实现。
