...
> 本文保留前期调研方案。实施时将原 P0 拆为“阶段 1：本地诊断”，原生 Langfuse 写入放在阶段 2，运行诊断与恢复放在阶段 3。当前可执行步骤以项目 README、phase-1.md 和 phase-2.md 为准；阶段 1 使用端口 14318、`OTEL_SEMCONV=codebuddy`，没有 Langfuse 出口。
...
0.1.1 修复并验收桌面持久插件和跨批 Session 关联。0.2.0 交付阶段 2A：agentlens 原生输入/输出 Token、按指定 Session 手动上传、发送账本、Langfuse v4 实际入库核验。为保留小步验证边界，将原生接入剩余的正文选择与缓存/费用核验放在 2B，自动持续上报和恢复放在阶段 3。详见 phase-2.md；尚未完成的部分不以架构方案代替实现。
