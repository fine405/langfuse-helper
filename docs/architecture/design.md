...
> 本文保留前期调研方案。实施时将原 P0 拆为“阶段 1：本地诊断”，原生 Langfuse 写入放在阶段 2，运行诊断与恢复放在阶段 3。当前可执行步骤以[项目首页](../../README.md)和[接入指南](../users/getting-started.md)为准；本文中的阶段端口、凭证位置与启动建议是当时的设计，不代表最终实现。
...
当前实现覆盖阶段 1、2A、2B 和 3：原生采集、Session 关联、可选正文与实际缓存/积分、自动增量上报、运行状态与恢复。正式架构以 [overview.md](overview.md) 为准；密钥由本地上报服务持有，Collector 保持本地元数据入口。运行状态采用已验证的 Hooks、任务文件终止标记和 worker 进程身份，没有另接未验证的私有 Trace API。

各项验证方式与证据见 [completion-plan.md](../developers/acceptance/completion-plan.md)。子代理通过原生结构兼容样本验证，普通任务、长工具、等待输入、并行工具和取消通过真实桌面任务验证，不把两类证据混称为同一种实测。
