...
import { root, configDir, dataDir } from './cli.mjs';
...
export async function enrichSession(records, sessionId) {
  ...
  const settings = { ...await readSettings(), content: registration.contentMode || 'metadata' };
  const transcript = new TranscriptStore(join(root, '.local/transcripts.sqlite'));
  try {
    let read;
